...
function getUpdateInfo() {
    var updateData = GetEditionInfo();
    if (updateData) {
        var data = JSON.parse(updateData);
        var param = {
            VersionExe: data.VersionExe,
            VersionAutoExe: data.VersionAutoExe
        };
        $.ajax({
            dataType: "json",
            ...
            url: LuckVipsoft.http + "/api/GeneralInterface/CheckUpdate",
            async: false,
            success: function (res) {
                ...
                $(".loadEffect").hide();
                if (res.data.strnodeDic) {
                    updateInfo = res.data.strnodeDic;
                    downloadUrl = res.data.posCashierUpdateUrl;
                    $("#updateinfo").html(res.data.updateTxt);
                    $("#testing").text("检查到新版本，点击\"下载\"下载更新文件。");
                    $("#tips").hide();
                    $(".updateBut").show();
                    $("#download").show();
                }
                else {
                    $("#testing").text("当前已是最新版本，无需更新。");
                }
            }
        });
    }
...
//下载更新文件
function updateDown() {
    $("#testing").text("下载完成，请单击“安装更新”完成系统升级。");
    $("#download").hide();
    $("#setupdate").show();
}