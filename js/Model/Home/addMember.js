layui.use(['layer', 'jquery', 'form', 'laydate'], function () {
        ...
        if (res.status == 1) {
            ...
            for (var i = 0; i < list.length; i++) {
                for (var g = 0; g < list2.length; g++) {
                    if (list[i].indexOf(list2[g].FieldName) != -1) {
                        if (list2[g].FieldValue == ('ShopID') && $.session.get('Cashier_User').MShopID == $.session.get('Cashier_User').ShopID) {
                            ...
                            <em>确认密码：</em>
                            <span>
                                <input name="addPassWord2" type="text" placeholder="再次输入会员密码" class="cd-form-input" lay-verify="repass" />
                            </span>
                                   </li>`
                        } else {
                            html += list[i]
    ...
    form.on('submit(addMem)', function (data) {
        ...
        var param = {
            ...
            CustomFieldsJson: customFieldsJson ? JSON.stringify(customFieldsJson) : ""
        }
